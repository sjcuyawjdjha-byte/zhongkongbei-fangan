...
\begin{reportframe}
    ...
    \mybody
    本机器人采用子母车架构，整体机械功能涵盖\textbf{全向移动底盘}与\textbf{物块推动机构}两大核心模块。下车（母车）承担移动与承载任务，搭载四组麦克纳姆轮实现全向运动；上车（子车）承担物块推动执行任务。整体采用双层 6061-T6 铝合金板材框架，上下层之间通过 M3 铜柱螺接为一体，子车经顶板腰型槽与锁紧螺栓固定于母车之上，可实现高速全向移动、稳定承载与精确推块。
    ...
    底板作为下车的动力基准，采用 6061-T6 铝合金板材激光切割而成。其表面预留了高密度的安装孔位阵列，四组驱动电机座通过 M3 螺栓固定于底板预留孔位，万向轮支撑件经定位销与螺栓双重定位连接。蓄电池通过扎带与防滑垫固定于底板中央低位区域，使整车重心处于安全包络内，显著提升了高速转向时的动态稳定性。

    \subsectiontitle{（2）顶板设计（见图 2）}
    顶板兼具结构加强与子车承载功能，通过四根 M3 铜柱与底板螺接形成刚性双层框架。子车通过腰型槽配合锁紧螺栓固定于顶板上表面。
...
\begin{reportframe}[breakable=false]
    ...
    以 \textbf{STM32H743ZIT6} 为主控核心，运行巡线 PID、状态机调度与视觉数据解析等全部上层逻辑；\textbf{STM32H750VBT6} 作为副控，专职负责机械臂步进电机脉冲输出与舵机 PWM 生成。主控与副控之间通过 UART 串口（TTL 电平直连）交换状态指令与坐标数据。\textbf{K230 视觉模块}同样经 UART 串口接入主控，以固定帧率输出目标物体的像素坐标。
    ...
    \textbf{315\,MHz 四路遥控模块}用于远程启停与紧急制动，接收端 IO 直连主控 GPIO 中断引脚，响应延迟 $<\!10$\,ms。整车线缆采用端子化接插件（XH2.54 / PH2.0），便于快速拆装与故障排查。
...
\begin{reportframe}
    ...
    \mybody
    软件系统承担机器人的全部自主决策与控制任务，主要实现四大核心功能：\textbf{全向底盘运动控制}、\textbf{红外巡线导航}、\textbf{视觉目标识别与定位}以及\textbf{机械臂抓取控制}。系统基于 STM32H743ZIT6 平台，采用 C 语言在 Keil MDK 环境下开发，以模块化方式组织代码，将上述功能分别封装为独立模块，由主程序状态机统一调度。设计目标为实现全自主完赛，无需人工干预，巡线偏差控制在 5\,mm 以内，视觉定位精度优于 1\,cm。

    \subsectiontitle{（1）整体架构}
    主程序运行于 STM32H743ZIT6 主控芯片上，通过有限状态机管理机器人行为，定义若干运行状态（如 \texttt{LINE\_FOLLOWING}、\texttt{VISION\_TRACKING}、\texttt{ARM\_GRABBING} 等），每个控制周期（10\,ms）根据当前状态调用对应的功能函数，实现任务的有序切换与执行。副控 STM32H750VBT6 接收主控指令，专职执行机械臂运动控制。
    ...
    K230 视觉模块通过串口中断方式向 STM32 发送目标坐标帧。主控解析坐标后，在视觉追踪状态下计算目标与图像中心的像素偏差，换算为底盘横移与旋转指令，驱动小车对准目标，随后切换至机械臂抓取状态。
...
\begin{reportframe}
    ...
    \mybody
    本机器人采用子母车架构，具备\textbf{全向移动、自主巡线、视觉识别、机械臂抓取}四大核心功能，启动后按照预设策略全自主完成比赛全程，无需人工遥控干预。整体流程分为以下阶段：

    \subsectiontitle{（1）启动阶段}
    裁判发出开始信号后，机器人完成 STM32 系统自检与各模块初始化，确认巡线传感器、K230 视觉模块及机械臂均处于正常工作状态，随即进入巡线行进模式，驶离启动区。

    \subsectiontitle{（2）目标搜寻阶段}
    机器人沿地面引导线巡线行驶，K230 视觉模块持续扫描周围环境。一旦识别到目标物体（方块或圆环），主控切换至视觉追踪模式，驱动底盘横移对准目标，精确定位后触发抓取流程。
    ...
    \begin{itemize}[leftmargin=2em, itemsep=3pt, parsep=0pt, topsep=4pt]
        ...
        \item \textbf{视觉定位精度}：K230 模块输出目标像素坐标，主控通过比例控制将底盘对准目标，减小抓取误差。
